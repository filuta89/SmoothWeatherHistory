...
{
    private function initializeSession(SessionInterface $session, EntityManagerInterface $em, LoggerInterface $logger): string
    {
        if (!$session->isStarted()) {
            $session->start();
            $this->deleteExpiredResponsesAction($em, $logger);
        }
    ...
    {
        try {
            $sessionId = $this->initializeSession($session, $em, $logger);
            
            
            $logger->info('----------------------------sessionId------------------------>' . print_r($sessionId, true));
            try {
                $sessionSpecificIds = $em->createQueryBuilder()
                    ->select('rcd.id')
                    ->from(ResponseCommonData::class, 'rcd')
                    ->where('rcd.sessionId = :sessionId')
                    ->setParameter('sessionId', $sessionId)
                    ->getQuery()
                    ->getArrayResult();

                $responseId = array_map(function($result) {
                    return $result['id'];
                }, $sessionSpecificIds);

                $logger->info('----------------------------array of old sessionSpecificIds------------------------>' . print_r($responseId, true));
     ...
     */
    public function deleteExpiredResponsesAction(EntityManagerInterface $em, LoggerInterface $logger): Response
    {
        $responseCommonDataRepo = $em->getRepository(ResponseCommonData::class);
        $weatherDataRepo = $em->getRepository(WeatherData::class);
        $activeResponseIds = $responseCommonDataRepo->findActiveResponseIds();

        $logger->info('----------------------------deleteExpiredResponsesAction - activeResponseIds------------------------>' . print_r($activeResponseIds, true));

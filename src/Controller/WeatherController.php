...
class WeatherController extends AbstractController
{
                                       ...
                                       SessionInterface $session): Response
    {
        try {
            ...
            try {
                ...
                $this->addFlash('error', 'There was an error executing the query built by the QueryBuilder.
                 This might happen if the database connection is lost or if there is a syntax error in the SQL generated
                  by the query builder: ' . $e->getMessage());
            }
        ...
        $form = $this->createForm(WeatherType::class);
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                try {
                    ...
                    $this->addFlash('error', 'An error occurred while handling the form submission: ' .
                        $e->getMessage());
                }
            }
    ...
    private function fetchWeatherDataFromAPI($form): array
    {
        try {
            ...
            $this->addFlash('error', 'There was a network/database connectivity issue or the API returned
             error response while fetching weather data from the API: ' . $e->getMessage());
            return [];
        } catch (\Exception $e) {
            $this->addFlash('error', 'An unexpected error occurred while fetching weather data from the
             API: ' . $e->getMessage());
            return [];
        }
    }
    ...
    private function saveWeatherDataToDatabase(array $data, $form, EntityManagerInterface $em, string $sessionId): int
    {
        try {
            ...
            $this->addFlash('error', 'Failed to save weather data to database. Failed to insert a duplicate
             value in a unique column of the database: ' . $e->getMessage());
            return -1;
        } catch (ORMException $e) {
            $this->addFlash('error', 'Failed to save weather data to database. There was a general error
             with the entity manager or the database: ' . $e->getMessage());
            return -2;
        } catch (InvalidArgumentException $e) {
            $this->addFlash('error', 'Failed to save weather data to database. There was an invalid
             argument passed to one of the functions: ' . $e->getMessage());
            return -3;
        } catch (\Exception $e) {
    ...
    private function getWeatherDataForResponseIds(EntityManagerInterface $em, array $responseIds): array
    {
        ...
        foreach ($responseIds as $responseId) {
            try {
                ...
                $this->addFlash('error', 'An error occurred while getting weather data for
    ...
    public function getWeatherData(EntityManagerInterface $em, int $responseId): array
    {
        try {
            ...
            $city = $responseCommonData ? (str_contains($responseCommonData->getCity(), ',') ?
                strstr($responseCommonData->getCity(), ',', true) :
                $responseCommonData->getCity()) : null;
